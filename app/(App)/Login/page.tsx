...
const RoleSlider: FC<{ isTeacher: boolean; onRoleChange: (v: boolean) => void }> = ({isTeacher, onRoleChange}) => (
    <div className="relative flex w-full p-1 bg-white/5 rounded-lg border border-white/10 overflow-hidden">
        <motion.div
            ...
            animate={{x: isTeacher ? '100%' : '0%'}} transition={{type: "spring", stiffness: 300, damping: 30}}/>
        <motion.button type="button" onClick={() => onRoleChange(false)}
                       className="relative z-10 w-1/2 py-2.5 text-center font-semibold transition-colors flex items-center justify-center gap-2"
                       whileHover={{scale: 1.05}} whileTap={{scale: 0.98}}><FaUserGraduate
            className={!isTeacher ? 'text-white' : 'text-gray-400'}/><span
            className={!isTeacher ? 'text-white' : 'text-gray-400'}>Học sinh</span></motion.button>
        <motion.button type="button" onClick={() => onRoleChange(true)}
                       className="relative z-10 w-1/2 py-2.5 text-center font-semibold transition-colors flex items-center justify-center gap-2"
                       whileHover={{scale: 1.05}} whileTap={{scale: 0.98}}><FaChalkboardTeacher
            className={isTeacher ? 'text-white' : 'text-gray-400'}/><span
            className={isTeacher ? 'text-white' : 'text-gray-400'}>Giáo viên</span></motion.button>
    </div>
);
...
const FinalLoginPage = () => {
    ...
    return (
        <div
                    ...
                    <div className="p-8 sm:p-12">
                        <motion.div variants={containerVariants} initial="hidden" animate="show">
                            ...
                            <form onSubmit={handleLogin} noValidate className="space-y-5">
                                ...
                                <AnimatePresence mode="wait">{formError && (
                                    <motion.p key="formError" initial={{opacity: 0}} animate={{opacity: 1}}
                                              exit={{opacity: 0}}
                                              className="text-red-400 text-center text-sm">{formError}</motion.p>)}{resetMessage && (
                                    <motion.p key="resetMsg" initial={{opacity: 0}} animate={{opacity: 1}}
                                              exit={{opacity: 0}}
                                              className="text-green-400 text-center text-sm">{resetMessage}</motion.p>)}</AnimatePresence>
            ...
            {/* SỬA: Đưa toàn bộ style của trang Login vào đây */}
            <style jsx global>{`
                ...
                .glowing-input-container:hover {
                    background-image: conic-gradient(from var(--gradient-angle), #10b981, #fde047, #a3e635, #22c55e, #10b981);
                ...
                .action-button-shine {
                    position: relative;
                    width: 100%;
                    background-image: linear-gradient(to right, #16a34a, #10b981);
                    color: white;
                    font-weight: bold;
                    padding: 14px;
                    border-radius: 8px;
                ...
                .action-button-shine:hover:not(:disabled) {
                    transform: translateY(-2px);
                    box-shadow: 0 10px 20px rgba(16, 185, 129, 0.25);
                }

                .action-button-shine:disabled {
                ...
                .action-button-shine:hover:not(:disabled)::before {
                    left: 100%;
                }
