...
const GlowingButton: FC<{
    ...
    return (
        <motion.button type={type || "button"} onClick={onClick} disabled={isDisabled}
            ...
            <div className="relative z-10 flex items-center justify-center h-6">
                <AnimatePresence mode="wait" initial={false}>
                    {isLoading ? (
                        ...
                        <motion.div key="content" initial={{opacity: 0, scale: 0.5}} animate={{opacity: 1, scale: 1}}
...
const Logo = React.memo(() => (<Link href="/public" className="flex items-center gap-3 text-white">
...
const NoticeBoard: FC<{ isTeacher: boolean }> = ({isTeacher}) => {
    ...
    return (<div><h3
        className={`text-xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r transition-all duration-300 ${isTeacher ? 'from-sky-400 to-cyan-400' : 'from-emerald-400 to-teal-400'}`}>Bảng
        Tin</h3>
        <div className="space-y-4"> {loading && <p className="text-sm text-slate-400">Đang tải tin tức...</p>} {error &&
            <p className="text-sm text-red-400">Không thể tải tin tức.</p>} {announcements?.docs.map(doc => {
            const data = doc.data();
...
const LoginPage = () => {
    ...
    const handlePasswordReset = async () => {
        if (!email) {
            setFormError("Vui lòng nhập email của bạn để reset mật khẩu.");
            return;
        }
        setLoading(true);
        setFormError('');
                        ...
                        <div className="p-6 sm:p-10">
                            <motion.div variants={containerVariants} initial="hidden" animate="show">
                                ...
                                <form onSubmit={handleLogin} noValidate className="space-y-5">
                                    ...
                                    <motion.div variants={itemVariants}><ThemedInput isTeacher={isTeacher}
                                                                                     type="password"
                                                                                     placeholder="Mật khẩu"
                                                                                     value={password}
                                                                                     onChange={(e) => setPassword(e.target.value)}
                                                                                     icon={FaLock} required/>
                                    </motion.div>
                                    <motion.div variants={itemVariants}><Captcha isTeacher={isTeacher}
                                                                                 code={captchaCode} input={captchaInput}
                                    ...
                                    <motion.div variants={itemVariants} className="pt-2">
                                        <GlowingButton type="submit"
                                                       ...
                                                       icon={FiLogIn} color={isTeacher ? 'cyan' : 'yellow'}
                                                       isLoading={loading}>Đăng Nhập</GlowingButton>
                                    </motion.div>
                                </form>
                            </motion.div>
                        </div>
                        <div
                            className="hidden md:flex flex-col bg-black/20 p-6 sm:p-10 space-y-6 lg:space-y-8 lg:border-l border-slate-800">
                            <motion.div variants={containerVariants} initial="hidden" animate="show">
                                <motion.div variants={itemVariants} className="flex-grow"><MockAppUI
