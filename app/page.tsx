...
// FIX: Import thêm kiểu 'Variants' từ framer-motion
import { motion, useAnimation, useScroll, useTransform, useMotionValueEvent, Variants } from "framer-motion";
...
// === ĐỊNH NGHĨA ANIMATION VARIANTS (ĐÃ FIX LỖI) ===
// FIX: Thêm kiểu 'Variants' để TypeScript hiểu đúng
...
// === UPDATED: COMPONENT "HOW IT WORKS" VỚI HOVER EFFECT ===
                    ...
                    <div className="relative flex flex-col items-center gap-20">
                        ...
                        {steps.map((step, index) => (
                            <motion.div key={step.title} variants={stepVariants} initial="hidden" animate={stepControls[index]} className="relative z-10 text-center flex flex-col items-center w-full">
                                {/* NEW: Thêm thẻ motion.div để chứa nội dung và áp dụng hover effect */}
                                <motion.div
                                    className="bg-slate-800/40 p-6 rounded-xl w-full flex flex-col items-center"
                                    whileHover={{ y: -8, scale: 1.03 }}
                                >
                                    <div className="bg-emerald-600 text-white w-20 h-20 rounded-full flex items-center justify-center shadow-lg mb-4 ring-8 ring-slate-900">{step.icon}</div>
                        ...
                        {steps.map((step, index) => (
                            <motion.div key={step.title} variants={stepVariants} initial="hidden" animate={stepControls[index]} className="relative z-10 text-center flex flex-col items-center w-1/3">
                                ...
                                <motion.div
                                    ...
                                    whileHover={{ y: -8, scale: 1.03 }}
                                >
                                    <div className="bg-emerald-600 text-white w-20 h-20 rounded-full flex items-center justify-center shadow-lg mb-6 ring-8 ring-slate-900">{step.icon}</div>
                                    <h3 className="text-xl font-semibold mb-2 text-white">{step.title}</h3>
...
// === COMPONENT CHÍNH CỦA TRANG (Không có lỗi) ===
            ...
            <header className="relative w-full overflow-hidden bg-gradient-to-br from-green-900/80 via-emerald-800/80 to-yellow-600/60">
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="flex items-center justify-between h-16">
                            ...
                            <div className="hidden md:flex items-center space-x-2">
                                ...
                                <motion.a href="/login" className="bg-yellow-400 text-green-900 font-semibold py-2 px-4 rounded-lg ml-4" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>Đăng nhập</motion.a>
                            </div>
                ...
                <div className="min-h-screen flex flex-col lg:flex-row items-center justify-center gap-10 px-4 md:px-8 max-w-7xl mx-auto pt-16 relative z-10">
                    <motion.div className="text-center lg:text-left text-white max-w-2xl lg:max-w-xl" variants={containerVariants} initial="hidden" animate="visible">
                        <motion.h1 variants={itemVariants} className="text-4xl md:text-5xl lg:text-6xl font-extrabold mb-4 leading-tight">Chinh phục Tiếng Anh cùng <span className="text-yellow-300">Grammatica</span></motion.h1>
                        <motion.p variants={itemVariants} className="text-lg md:text-xl text-white/80 mb-8">Nền tảng học tập hiện đại dành riêng cho sinh viên, kết hợp bài học tương tác và công nghệ AI đột phá.</motion.p>
